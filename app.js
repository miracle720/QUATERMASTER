...
const BANK_DETAILS_START_DATE='2026-09-15';
...
function assignDeviceForCheckin(workerName, device){
  ...
  const sNum = device==='mobile' ? null : parseInt(device, 10);
  const log = {
    workerName,
...
// so the hand-off happens automatically and immediately — and also on a
// fixed interval (see bottom of file) as a self-heal fallback, since the
// data-change trigger only fires if some client happens to be connected at
// the exact moment a worker's order becomes next-up.
...
// Self-heal safety net for order timers: reconcileOrderTimers() is normally
// driven by the `orders` value listener, so it only runs when *some*
// connected client happens to see an orders change at the moment a worker's
// order becomes next-up. If no client is connected right then, and no
// further unrelated orders change happens afterward, that worker's card can
// get stuck on "Starting your timer…" indefinitely — nothing else ever
// re-triggers the check. Re-running it on an interval, from every connected
// client, guarantees any stuck worker gets picked up within a few seconds by
// whichever tab is open (this device, another worker's, or an admin's),
// instead of depending on an unrelated write to `orders` to happen to fire.
setInterval(()=>safeRender('reconcileTimers',()=>reconcileOrderTimers()),5000);
